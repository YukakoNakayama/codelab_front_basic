...
console.log(multipliedCalc(5,8));
...
// 理由：149行目でvarを使用することによってローカル変数にし、アクセスできないようにしたため。
...
// 理由：162行目でvarを使用していないためグローバル変数となり、どこからでもアクセスし書き換えできるようになっているため。
country = '日本';
...
// 理由:グローバル汚染の防止→即時関数で囲むと変数はローカル変数となるため、関数外の変数を書き換えるリスクがなくなる。
(function (){
  var shop = "コンビニ";
  console.log(shop);
}());
